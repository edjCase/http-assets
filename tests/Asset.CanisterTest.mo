...
shared ({ caller = owner }) actor class () = this_canister {
  ...
  func get_certified_endpoints() : Iter.Iter<CertifiedAssets.EndpointRecord> {
    CertifiedAssets.endpoints(assets_internal.fs.certs);
  };
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      ts_assert_or_print((assets.list_permitted({ permission = #Manage })) == [], "ManagePermissions permission should be empty");
      ...
      ts_assert_or_print(
        ...
        "Unexpected error granting #Manage permission from owner to manager",
      );

      ts_assert_or_print((assets.list_permitted({ permission = #Manage })) == [manager], "Only the manager should have ManagePermissions permission");
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      for (manager_or_controller in [manager, controller].vals()) {
        ...
        await* permit(manager_or_controller, #Manage, random_principal);
        await* revoke(manager_or_controller, #Manage, random_principal);
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      for (non_manager in [no_permission, preparer, committer].vals()) {
        ...
        let prepare_permissions = assets.list_permitted({
          permission = #Prepare;
        });
        ...
        let commit_permissions = assets.list_permitted({
          permission = #Commit;
        });
        ...
        let manage_permissions = assets.list_permitted({
          permission = #Manage;
        });
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print } : CanisterTests.TestTools) : async () {
      ...
      ts_assert(
        Result.isOk(
          assets.delete_batch(owner, { batch_id })
        )
      );
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      let actual_hello_html = assets.get({
      ...
      let actual_hello_index_html = assets.get({
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      for (principal_with_preparer_permission in [preparer, committer].vals()) {
        ...
        ts_assert(
          Option.isSome(
            Map.get(assets_internal.upload.batches, nhash, batch_id)
          )
        );

        ts_assert(
          Result.isOk(
            assets.delete_batch(principal_with_preparer_permission, { batch_id })
          )
        );
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      test_create_chunk_batch_id := switch (assets.create_batch(committer, {})) {
        case (#ok({ batch_id })) batch_id;
        case (#err(msg)) {
          ts_assert(false);
          return ts_print("Error creating batch: " # msg);
      ...
      ts_assert(
        Map.get(assets_internal.upload.chunks, nhash, chunk_ids.get(0)) == ?{
          content = chunk_1;
          batch_id;
        }
      );
      ...
      ts_assert(
        Map.get(assets_internal.upload.chunks, nhash, chunk_ids.get(1)) == ?{
          content = chunk_2;
          batch_id;
        }
      );

      let batch = switch (Map.get(assets_internal.upload.batches, nhash, batch_id)) {
        case (?batch) batch;
        case (null) {
          ts_assert(false);
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print } : CanisterTests.TestTools) : async () {
      ...
      for (chunk_id in Vector.vals(batch.chunk_ids)) {
        ts_assert(
          Option.isNull(
            Map.get(assets_internal.upload.chunks, nhash, chunk_id)
          )
        );
      };
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print } : CanisterTests.TestTools) : async () {
      ...
      test_create_asset_batch_id := switch (assets.create_batch(committer, {})) {
        case (#ok({ batch_id })) batch_id;
        case (#err(msg)) {
          ts_assert(false);
          return ts_print("Error creating batch: " # msg);
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print; ts_ok_or_print } : CanisterTests.TestTools) : async () {
      ...
      let actual_asset_content = assets.get({
        key = "/test/asset/hello";
        accept_encodings = ["identity"];
      });
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      let actual_asset_details = assets.get({
        key = "/test/asset/multiple-hello";
        accept_encodings = ["identity"];
      });
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      //             ts_print("encoding: " # debug_show ({ total_length = encoding.total_length; certified = encoding.certified; sha256 = encoding.sha256; modified = encoding.modified; content_chunks = null }));
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      let actual_identity_encoding_details = assets.get({
        key = "/test/asset/hello";
        accept_encodings = ["identity"];
      });
      ...
      ts_assert(
        Result.isOk(
          assets.delete_batch(committer, { batch_id = test_create_asset_batch_id })
        )
      );
    },
  );
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      let actual_gzip_encoding_details = assets.get({
        key = "/test/commit/file.json";
        accept_encodings = ["gzip"];
      });
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      let actual_hello_asset_details = assets.get({
        key = "/test/asset/hello";
        accept_encodings = ["identity"];
      });
      ...
      ts_assert_or_print(
        ...
        "[0xv3] Failed to revert changes to /test/asset/hello (actual, expected): " # debug_show (actual_hello_asset_details, expected_hello_asset_details),
      );

      // able to delete the batch because the commit failed
      ts_assert(
        Result.isOk(
          assets.delete_batch(committer, { batch_id })
        )
      );
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      let #ok({ chunk_ids }) = await* assets.create_chunks(preparer, { batch_id; content = chunks }) else return ts_print("Failed to create chunks");
      ...
      ts_assert_or_print(
        Option.isNull(Map.get(assets_internal.upload.batches, nhash, batch_id)),
        "[0xv7] Proposed batch should be removed after committing",
      );

      // not able to delete the batch because it has already been committed
      ts_assert_or_print(
        Result.isErr(
          assets.delete_batch(committer, { batch_id })
        ),
        "Should through an error when trying to delete a batch that should no longer exist",
      );
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      // ts_assert_or_print(
      //     Option.isNull(Map.get(assets_internal.upload.chunks, nhash, chunk_id)),
      //     "[0xv7] Chunks should be removed after committing",
      // );
        ...
        ts_assert_or_print(
          not Itertools.any(
            ...
            func(asset_details : CertifiedAssets.EndpointRecord) : Bool {
              asset_details.url == asset and asset_details.status == 200;
            },
          ),
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      let region = assets_internal.shared_region;

      let capacity = MemoryRegion.capacity(region);
      let allocated = MemoryRegion.allocated(region);

      Debug.print("memory-region stats: " # debug_show (MemoryRegion.memoryInfo(region)));

      let deallocated_blocks_in_range = Iter.toArray(
        MemoryRegion.deallocatedBlocksInRange(region, 0, capacity)
      );

      Debug.print("deallocated_blocks_in_range: " # debug_show (deallocated_blocks_in_range));

      let allocated_blocks = if (allocated == 0)[] else Iter.toArray(
        MemoryRegion.allocatedBlocksInRange(region, 0, capacity + 1)
      );

      Debug.print("allocated_blocks: " # debug_show (allocated_blocks));

      ts_assert_or_print(
        allocated == 0,
        "Memory leak detected in region ",
      );
  ...
  suite.add(
    "verify asset canister is empty",
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      let stored_assets = FileSystem.to_array(assets_internal.fs);

      ts_assert_or_print(
        stored_assets.size() == 0,
        "FileSystem should be empty but got: " # debug_show Array.map<(Text, Any), Text>(stored_assets, func(key : Text, asset : Any) : Text = key),
      );

      ts_assert_or_print(
        Map.size(assets_internal.upload.batches) == 0,
        "Batches should be empty but got: " # debug_show (Iter.toArray(Map.keys(assets_internal.upload.batches))),
      );

      ts_assert_or_print(
        Map.size(assets_internal.upload.chunks) == 0,
        "Chunks should be empty but got: " # debug_show (Map.toArray(assets_internal.upload.chunks)),
      );

      let assets_list = assets.list({});
      ts_assert_or_print(
        assets_list == [],
        "Asset canister should be empty but got: " # debug_show (assets_list),
      );
    },
  );
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      let prepare_principals = assets.list_permitted({
        permission = #Prepare;
      });
      ...
      let commit_principals = assets.list_permitted({
        permission = #Commit;
      });

      ts_assert_or_print(
        commit_principals == [owner, canister_id],
      ...
      let manage_permissions_principals = assets.list_permitted({
        permission = #Manage;
      });
  ...
  suite.add(
    ...
    func({ ts_assert; ts_print; ts_assert_or_print } : CanisterTests.TestTools) : async () {
      ...
      ts_assert(assets.list_permitted({ permission = #Manage }) == []);
